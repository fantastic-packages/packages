...
2. exec `gen_new_apksign_key.sh` and `gen_new_gpg_key.sh` generate apksign&gpg key for yourself
   ...
   + `./keys/gpg/*.rev`
   + `./keys/gpg/*.sec`
3. goto `https://github.com/settings/personal-access-tokens`, add a new token with `write` access for `Contents` `Workflows` permissions
4. goto `https://github.com/<your_fork>/settings/secrets/actions`, add your secret keys
   + `RELEASES_PUSH`: <your_personal_access_tokens>
...
6. if you need to use other external feeds to build a image, you need to put their apksign pub-keys in the specified location and add them to the `VERIFY_KEY_ID`
...
7. modify [ version, arch, target, profile, no_img ] matrix to replace the target devices
...
8. adjust the imagebuilder preinstalled packages
